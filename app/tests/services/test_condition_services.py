...
from app.services.condition_service import get_condition_by_id, get_all_conditions, get_conditions_by_encounter_id
...
def test_get_condition_by_id_returns_none_for_missing_condition(mocker):
   ...
   mocker.patch('app.services.condition_service.supabase.table', return_value=mock_table)
...
def test_get_all_conditions_returns_list(mocker):
   ...
   fake_response = mocker.Mock()
   fake_response.data = [
       {"id": str(uuid4()), "encounter_id": str(uuid4()), "condition_name": "Hypertension", "severity": "High"},
       {"id": str(uuid4()), "encounter_id": str(uuid4()), "condition_name": "Diabetes", "severity": "Medium"},
   ]
...
def test_get_conditions_by_encounter_id_returns_list(mocker):
   ...
   fake_response = mocker.Mock()
   fake_response.data = [
       {"id": str(uuid4()), "encounter_id": str(uuid4()), "condition_name": "Asthma", "severity": "Low"},
       {"id": str(uuid4()), "encounter_id": str(uuid4()), "condition_name": "Allergy", "severity": "Medium"},
   ]
