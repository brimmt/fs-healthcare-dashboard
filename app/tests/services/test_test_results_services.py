...
from app.services.test_result_service import get_test_results_by_id, get_all_test_results,get_test_results_by_encounter_id
...
def test_get_test_result_by_id_returns_none_for_missing_test_result(mocker):
   ...
   mocker.patch('app.services.test_result_service.supabase.table', return_value=mock_table)
...
def test_get_all_test_results_returns_list(mocker):
   ...
   fake_response = mocker.Mock()
   fake_response.data = [
       {"id": str(uuid4()), "encounter_id": str(uuid4()), "test_name": "Blood Test", "result": "Normal"},
       {"id": str(uuid4()), "encounter_id": str(uuid4()), "test_name": "X-Ray", "result": "No Issues"},
   ]
...
def test_get_test_results_by_encounter_id_returns_list(mocker):
    ...
    fake_response = mocker.Mock()
    fake_response.data = [
         {"id": str(uuid4()), "encounter_id": str(encounter_id), "test_results": "Abnormal", "created_at":"2024-01-01T00:00:00Z"},
         {"id": str(uuid4()), "encounter_id": str(encounter_id), "test_results": "Normal", "created_at": "2024-01-01T00:00:00Z"},
    ]
    ...
    assert all(str(encounter_id) == test_result["encounter_id"] for test_result in result)
