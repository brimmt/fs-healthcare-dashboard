...
from app.services.encounter_service import get_all_encounters as service_get_all_encounters, get_encounter_by_id as service_get_encounter_by_id, get_encounters_by_patient_id as service_get_encounters_by_patient_id, get_encounters_by_hospital_name as service_get_encounters_by_hospital_name, get_encounters_by_doctor_name as service_get_encounters_by_doctor_name      
...
async def get_encounters_by_patient_id(patient_id: UUID):
    ...
    if result is None or len(result) == 0:
        raise HTTPException(
            status_code=404, 
            detail="No encounters found for the specified patient ID"
            )
    return result
...
async def get_encounters_by_doctor_name(doctor_name: str):
    ...
    if result is None or len(result) == 0:
        raise HTTPException(
            status_code=404, 
            detail="No encounters found for the specified doctor name"
            )
    return result
